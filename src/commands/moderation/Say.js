...
module.exports = class extends Command {
    constructor(client) {
        super(client, {
            name: 'say',
            descriptions: 'Faz com que o bot diga alguma mensagem.',
            options: [
                /*{
                    name: 'canal',
                    type: 'CHANNEL',
    ...
    run = async (interaction) => {
        ...
        const collector = reply.createMessageComponentCollector({ filter, max: 1, time: (3 * 6000) })
        ...
        collector.on('collect', (i) => {
            ...
            const embed = new MessageEmbed()
                .setTitle('Uma nova mensagem foi enviada neste canal!')
        ...
        collector.on('end', (collected, reason) =>{
            if (reason === 'time') interaction.editReply({content: 'O tempo para informar o canal esgotou!', components:[]})
        })
    }
}
